 ...
 */
 ...
 class QS_CF7_helpers{
     ...
     public static function qs_get_template_part( $slug, $name = '' ) {
         ...
         if ( $name  ) {
             $template = locate_template( array( "{$slug}-{$name}.php", QS_CF7_CRM_IN_CLOUD_TEMPLATE_PATH . "{$slug}-{$name}.php" ) );
         }

         if ( ! $template && $name && file_exists( QS_CF7_CRM_IN_CLOUD_TEMPLATE_PATH . "/templates/{$slug}-{$name}.php" ) ) {
             $template = locate_template( array( "{$slug}-{$name}.php", QS_CF7_CRM_IN_CLOUD_TEMPLATE_PATH . "/templates/{$slug}-{$name}.php" ) );
         }
